...
- データ出典：SchaleDB、ブルアカ攻略 wikiru（生徒名・プロフィール・立ち絵URL）
- 画像出典：SchaleDB（icon / portrait）、wikiru（立ち絵）

## 権利・ガイドラインに関する注意（免責）

本リポジトリおよびゲーム「シャーレ暗号解読」は、ブルーアーカイブのファンによる**二次創作物**です。

- 株式会社Yostar、NEXON Games、および「ブルーアーカイブ」運営とは**一切関係がなく、公式に許諾・承認されたものではありません（非公式・非公認）**。
- 「ブルーアーカイブ」および生徒名・キャラクター・画像等に関する著作権・商標等の権利は、すべて権利者に帰属します。
- 本プロジェクトは**非営利**であり、ブルーアーカイブの**二次創作ガイドラインの範囲内での利用を意図**しています。**公式ガイドラインに抵触する意図はありません。**
- 立ち絵・アイコン・立ち絵画像等はリポジトリに同梱せず、利用者が取得スクリプトを実行して各自取得する構成です（画像そのものの再配布は行いません）。
- データはファンサイトを参照した非公式の参考情報であり、内容の正確性・最新性を保証しません。
